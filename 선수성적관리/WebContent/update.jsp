...
<body>
	...
	<%
		...
		sql=" select PLAYER_NAME,TEAM_NO,PLAYER_PHONE,";
		sql+=" to_char(PLAYER_BIRTH,'yy/mm/dd') PLAYER_BIRTH,";
		sql+=" nvl(PLAYER_EMAIL,' ') PLAYER_EMAIL,";
		sql+=" nvl(BACK_NO, 0) BACK_NO";
		sql+=" from PLAYER_TBL_09";
		sql+=" where PLAYER_NO=?";
		ps=con.prepareStatement(sql);
		ps.setInt(1, PLAYER_NO);
		rs=ps.executeQuery();
		rs.next();
		
		String PLAYER_NAME = rs.getString("PLAYER_NAME");
		int TEAM_NO = rs.getInt("TEAM_NO");
		String PLAYER_PHONE = rs.getString("PLAYER_PHONE");
		String PLAYER_BIRTH = rs.getString("PLAYER_BIRTH");
		String PLAYER_EMAIL = rs.getString("PLAYER_EMAIL");
	...
	<section>
		...
		<form method="post" action="insertpro.jsp" name="f">
			<table style="margin: 0 auto;" width="400" border="1">
				...
				<tr>
					<th>이름</th>
					<td><input type="text" name="PLAYER_NAME" value="<%=PLAYER_NAME %>" size="20" maxlength="20"></td>
				</tr>
				<tr>
					<th>소속팀번호</th>
					<td><input type="text" name="TEAM_NO" value="<%=TEAM_NO %>" size="20"></td>
				</tr>
				<tr>
					<th>연락처</th>
					<td><input type="text" name="PLAYER_PHONE" value="<%=PLAYER_PHONE %>" size="20" maxlength="14"></td>
				</tr>
				<tr>
					<th>생년월일</th>
					<td><input type="text" name="PLAYER_BIRTH" value="<%=PLAYER_BIRTH %>" size="20"></td>
				</tr>
				<tr>
					<th>메일주소</th>
					<td><input type="text" name="PLAYER_EMAIL" value="<%=PLAYER_EMAIL %>" size="20" maxlength="50"></td>
				</tr>
				<tr>
					<th>등번호</th>
					<td><input type="text" name="BACK_NO" <%if(BACK_NO!=0){out.println("value='"+BACK_NO+"'");}%> size="20"></td>
				</tr>
			</table>
			<div>
